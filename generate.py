import os
from argparse import ArgumentParser
import torch

from src.datasets import datasets_map, DATASET_MAP
from src.datasets.dataset_loader import load_asr_data
from src.models import models_map
import json
from tqdm import tqdm


torch.backends.cuda.enable_mem_efficient_sdp(False)
torch.backends.cuda.enable_flash_sdp(False)

OUTPUT_DIR = './result'


def set_gpu(gpu_id):
    """
    设置要使用的 GPU。

    Args:
        gpu_id: 要使用的 GPU 的 ID (整数)。
    """
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    print(f"Set CUDA_VISIBLE_DEVICES to: {os.environ['CUDA_VISIBLE_DEVICES']}")


def main():
    parser = ArgumentParser()
    parser.add_argument('--model', type=str, default='qwen2', choices=list(models_map.keys()))
    parser.add_argument('--model_path', type=str, default='')
    parser.add_argument('--task', type=str, default='asr')
    # parser.add_argument('--gpu', type=int, default=0)
    args = parser.parse_args()

    # 将任务运行在指定GPU上
    task = args.task
    # 遍历打印参数
    for key, value in vars(args).items():
        print(f'{key}: {value}')

    if task == 'asr':
        data = load_asr_data()
    else:
        data = datasets_map[task](DATASET_MAP[task], task)
    # load data
    # data = load_dataset('hlt-lab/voicebench', args.data, split=args.split)
    # data = data.cast_column("audio", Audio(sampling_rate=16_000))

    # load model
    model = models_map[args.model](args.model_path)
    # data = data.select([0,1,2,3,4,5])

    # if args.modality == 'ttft':
    #     # avoid cold start
    #     _ = model.generate_ttft(data[0]['audio'])

    # inference
    results = []
    for item in tqdm(data, total=len(data)):
        audio = item['audio']
        prompt = item['prompt']
        pred = model.prompt_mode(prompt, audio)
        results.append({
            'file': item['file'],
            'prompt': prompt,
            'instruction': item['instruction'],
            'pred': pred,
            'target': item['output']
        })

    # save results
    output_file = f'{args.model}-{args.task}.jsonl'
    with open(output_file, 'w') as f:
        for record in results:
            json_line = json.dumps(record)  # Convert dictionary to JSON string
            f.write(json_line + '\n')


if __name__ == '__main__':
    main()
